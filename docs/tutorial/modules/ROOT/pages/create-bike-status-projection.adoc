...
In this case, we will implement support in our application to return information about one or more bikeCollection, including where the bike is, whether it is available or rented and who has rented it.
...
To keep the list of our bikeCollection in the query model up to date, we need to define a method that will be invoked whenever a new bike is registered in the system (the `BikeRegisteredEvent` represents that notification). We can do this by adding an `@EventHandler` method to our `BikeStatusProjection`:
...
=== Implement a query to return all the bikeCollection.

Let's start by implementing a method to return all the bikeCollection (with their status) defined in our system. Add the following method to your `BikeStatusProjection` class:
...
<.> Since our query has no parameters (we want to retrieve the information for *all the bikeCollection* in our system), our query handler method does not receive any parameters. It only needs to return the list of items we find in our DB.
...
In short, we have defined a query handler method that Axon Framework will call upon the reception of a query message to `FIND_ALL` the bikeCollection in our system. And the method will simply retrieve the up-to-date information from the DB and return the `BikeStatus` for all the bikeCollection.


=== Implementing support for other queries in our projection.

We may need to support different query requests for information about the bikeCollection in our system. The same projection can be used to satisfy different queries.

For example, if we want to support queries to return all the available bikeCollection, filtering by the type, or the `BikeStatus` for a specific bike by its `bikeId`, we can add the following two methods to our `BikeStatusProjection`:
...
Now, our `BikeStatusProjection` fully supports answering to queries to `findAll` bikeCollection, `findAvailable` bikeCollection of a certain type, and `findOne` specific bike given its `bikeId`.
...
To implement the method that exposes the endpoint for returning all the bikeCollection and their status, add the following method to our `RestController`:
...
----
<.> The `GetMapping` Spring annotation specifies that this method will be invoked whenever a `GET` request to the URL `/bikeCollection` is received by the application.
...
----
<.> The `@GetMapping` annotation configures the method to be invoked when a `GET` request to `/bikeCollection/{bikeId}` is received, and defines the part of the URL that comes after `/bikeCollection/` to be assigned to the `bikeId` path variable.
...
[NOTE]
====
When we invoked the endpoint to register new bikeCollection after we implemented the command handler, the command handler triggered the corresponding `BikeRegisteredEvent` to notify all the components (like our projection) of the changes.
...
[,console]
----
% curl -X GET "http://localhost:8080/bikeCollection"
[
  {
...
[,console]
----
%% curl -X GET "http://localhost:8080/bikeCollection/8427681b-1ee6-4e0a-b5d8-c524b9ed553d"
{
  "bikeId": "8427681b-1ee6-4e0a-b5d8-c524b9ed553d",
  "bikeType": "city",
  "location": "Utrecht",
...
GET {{rental}}/bikeCollection
Accept: application/json

### Bike status
# Show bike status
GET {{rental}}/bikeCollection/8427681b-1ee6-4e0a-b5d8-c524b9ed553d
Accept: application/json
...
[source,console]
----
GET http://localhost:8080/bikeCollection
