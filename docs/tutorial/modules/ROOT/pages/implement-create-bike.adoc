...
When the application receives a request to create a bike, that implies that the state of our application will change because, after successfully processing it, we will have a new bike in our system (so, the list of available bikeCollection -our application's state- will have changed). So, we will model our request as a `Command`; in our case, we will call it `RegisterBikeCommand`.
...
For this endpoint, we will consider the following request format:
    ...
    POST /bikeCollection?bikeType={bikeType}&location={city}
